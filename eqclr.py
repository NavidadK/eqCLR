...
IMG_RESIZE = 129                                      # rescaling image size. If None, use original size
N = 4                                                 # group size for equivariant model
MODEL_FILENAME = f"eqCLR_resnet18_N{N}_bloodmnist"    # base filename 
...
# rotations only (cyclic group)
model = EqResNet18(N=N, r2_act=gspaces.rot2dOnR2(N), in_channels=IN_CHANNELS, maxpool=MAXPOOL, projector_hidden_size=PROJECTOR_HIDDEN_SIZE, n_classes=PROJECTOR_OUTPUT_SIZE)
    
# joint flip-roations (dyhedral group)
# model = EqResNet18(N=int(N/2), r2_act=gspaces.flipRot2dOnR2(int(N/2)), in_channels=IN_CHANNELS, maxpool=MAXPOOL, projector_hidden_size=PROJECTOR_HIDDEN_SIZE, n_classes=PROJECTOR_OUTPUT_SIZE)

# Mixed eqCLR (2 eq layer)
# rotations only (cyclic group)
# model = EqResNet(N=N, in_channels=IN_CHANNELS, eq_blocks=2, projector_hidden_size=PROJECTOR_HIDDEN_SIZE, n_classes=PROJECTOR_OUTPUT_SIZE)

# joint flip-roations (dihedral group)
# model = EqResNet(N=int(N/2), r2_act=gspaces.flipRot2dOnR2(int(N/2)), in_channels=IN_CHANNELS, eq_blocks=2,  projector_hidden_size=PROJECTOR_HIDDEN_SIZE, n_classes=PROJECTOR_OUTPUT_SIZE)

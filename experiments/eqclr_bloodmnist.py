...
folder = 'results/bloodmnist'
name_data = 'bloodmnist_129'
N_CLASSES = 8
IN_CHANNELS = 3
IMG_RESIZE = 129  
...
for seed in range(3):
    ...
    for group in groups:
        for N in [2, 4, 8]:
            ...
            MODEL_FILENAME = f"C-0001_seed{seed}_{name_data}-eqCLR_resnet18_N{N}_{group}_kp_100epochs_b512_eval"
            ...
            if group == 'flip_rot':
                if N == 1:
                    ...
                    N = int(N/2)  # for flip_rot, N is halved because of the flip symmetry  

            if group == 'flip_rot':
                model = EqResNet18(N=N, r2_act=gspaces.flipRot2dOnR2(N), in_channels=IN_CHANNELS, maxpool=MAXPOOL, projector_hidden_size=PROJECTOR_HIDDEN_SIZE, n_classes=PROJECTOR_OUTPUT_SIZE, adjust_channels='keep_param')
            elif group == 'rot':
        ...
        for N in [2, 4, 8]: 
            ...
            MODEL_FILENAME = f"0002_seed{seed}_{name_data}-mixed_eqCLR_resnet18_2eqblocks_N{N}_{group}_kp_1000epochs"
            print(f"Model filename: {MODEL_FILENAME}")
            ...
            ###################### NETWORK ARCHITECTURE #########################
            if group == 'flip_rot':
                model = EqResNet(N=N, r2_act=gspaces.flipRot2dOnR2(N), in_channels=IN_CHANNELS, eq_blocks=2)
            elif group == 'rot':

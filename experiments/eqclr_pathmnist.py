...
folder = 'results/pathmnist'
name_data = 'pathmnist_129'
N_CLASSES = 9
IN_CHANNELS = 3
IMG_RESIZE = 129  # if None, use original size 28x28
...
for seed in range(3):
    ...
    data_train = PathMNIST(split='train', download=False, size=128, root='data/pathmnist128/', transform=transform)
    data_test = PathMNIST(split='test', download=False, size=128, root='data/pathmnist128/', transform=transform)
    data_val = PathMNIST(split='val', download=False, size=128, root='data/pathmnist128/', transform=transform)
    print("Data loaded.")
    ...
    data_train_ssl = PathMNIST(split='train', download=False, size=128, root='data/pathmnist128/', transform=paired_ssl_transforms)
    ...
    data_train_classifier = PathMNIST(split='train', download=False, size=128, root='data/pathmnist128/', transform=transforms_classifier)
    ...
    # EqCLR
    for group in ['rot', 'flip_rot']:
        for N in [2, 4, 8]:
            
            ###################### NETWORK ARCHITECTURE #########################

            MODEL_FILENAME = f"0003_seed{seed}_{name_data}-eqCLR_resnet18_N{N}_{group}_kp_1000epochs_lr{BASE_LR}"
        ...
        for N in [2, 4, 8]:# [4, 8]:
            ...
            MODEL_FILENAME = f"C-0002_seed{seed}_{name_data}-mixed_eqCLR_resnet18_2eqblocks_N{N}_{group}_kp_100epochs_b512"
            print(f"Model filename: {MODEL_FILENAME}")

            ###################### NETWORK ARCHITECTURE #########################
            if group == 'flip_rot':
                model = EqResNet(N=int(N/2), r2_act=gspaces.flipRot2dOnR2(int(N/2)), in_channels=IN_CHANNELS, eq_blocks=2)
            elif group == 'rot':
                model = EqResNet(N=N, in_channels=IN_CHANNELS, eq_blocks=2)

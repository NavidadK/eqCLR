...
###################### PARAMS ##############################
folder = 'results/bloodmnist/sample_efficiency'
N_CLASSES = 8
IN_CHANNELS = 3
IMG_RESIZE = 33
...
for rs in range(3):
    ...
    MODEL_FILENAME = f"001-seed{rs}_bloodmnist33-eqCLR_N4"
    ...
    if EFFECTIVE_EPOCHS:
        target_updates = (len(data_train) // BATCH_SIZE_OG) * N_EPOCHS_OG
    ...
    for frac in fractions:
        ...
        if EFFECTIVE_BATCH:
            effective_batch_size = min(BATCH_SIZE_OG, len(data_train_ssl_frac))
        ...
        if EFFECTIVE_EPOCHS:
            ...
            N_EPOCHS = N_EPOCHS_OG
            print(f"Using constant epochs: {N_EPOCHS}")


        model = EqResNet18(N=4, in_channels=IN_CHANNELS, maxpool=MAXPOOL, projector_hidden_size=PROJECTOR_HIDDEN_SIZE, n_classes=PROJECTOR_OUTPUT_SIZE, adjust_channels='keep_param')
